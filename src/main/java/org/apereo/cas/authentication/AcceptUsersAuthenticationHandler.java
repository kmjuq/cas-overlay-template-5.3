...
public class AcceptUsersAuthenticationHandler extends AbstractUsernamePasswordAuthenticationHandler {
    ...
    protected AuthenticationHandlerExecutionResult authenticateUsernamePasswordInternal(final UsernamePasswordCredential credential,
        ...
        if (strategy != null && StringUtils.isNotBlank(username)) {
            log.debug("Attempting to examine and handle password policy via [{}]", strategy.getClass().getSimpleName());
            val principal = this.principalFactory.createPrincipal(credential.getId(),new HashMap<String,Object>(){{
                put("xbm", "男");
                put("mail", "kemengjian@126.com");
                put("yhlbdm", "L0000");
            }});
